...
/**
 * FFTiltshelf - Wraps FFmpeg's 'tiltshelf' audio filter
 * Apply a tilt shelf filter (boosts highs while cutting lows, or vice versa)
 * FFmpeg filter: tiltshelf=frequency=<f>:width_type=<t>:width=<w>:gain=<g>:poles=<p>:mix=<m>:...
 */
class FFTiltshelf : public BaseFilter {
    Q_OBJECT
public:
    FFTiltshelf();
    ~FFTiltshelf() override = default;

    QString displayName() const override { return "Tilt Shelf"; }
    QString filterType() const override { return "ff-tiltshelf"; }
