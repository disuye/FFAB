...
/**
 * FFLowshelf - Wraps FFmpeg's 'lowshelf' audio filter
 * Apply a low shelf filter
 * FFmpeg filter: lowshelf=frequency=<f>:width_type=<t>:width=<w>:gain=<g>:poles=<p>:mix=<m>:...
 */
class FFLowshelf : public BaseFilter {
    Q_OBJECT
public:
    FFLowshelf();
    ~FFLowshelf() override = default;

    QString displayName() const override { return "Low Shelf"; }
    QString filterType() const override { return "ff-lowshelf"; }
