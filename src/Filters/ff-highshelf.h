...
/**
 * FFHighshelf - Wraps FFmpeg's 'highshelf' audio filter
 * Apply a high shelf filter
 * FFmpeg filter: highshelf=frequency=<f>:width_type=<t>:width=<w>:gain=<g>:mix=<m>:...
 */
class FFHighshelf : public BaseFilter {
    Q_OBJECT
public:
    FFHighshelf();
    ~FFHighshelf() override = default;

    QString displayName() const override { return "High Shelf"; }
    QString filterType() const override { return "ff-highshelf"; }
