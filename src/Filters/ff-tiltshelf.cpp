...
FFTiltshelf::FFTiltshelf() {
    position = Position::MIDDLE;
    updateFFmpegFlags();
}

QWidget* FFTiltshelf::getParametersWidget() {
    ...
    // Frequency parameter (Hz) - pivot point
    auto frequencyGroup = new QGroupBox("Pivot Frequency (Hz)");
    ...
    tiltLayout->addWidget(frequencyGroup);

    // Gain/Tilt parameter (dB) - positive = boost highs/cut lows
    auto gainGroup = new QGroupBox("Tilt (dB)");
    ...
    auto gainMinLabel = new QLabel("-24 (bright)"); // values reversed for some reason?
    ...
    auto gainMaxLabel = new QLabel("+24 (warm)"); // values reversed for some reason?
    gainControlLayout->addWidget(gainMaxLabel);

    gainLayout->addLayout(gainControlLayout);
    tiltLayout->addWidget(gainGroup);
    ...
    tiltLayout->addWidget(widthTypeGroup);
    ...
    tiltLayout->addWidget(widthGroup);
    ...
    tiltLayout->addWidget(polesGroup);
...
void FFTiltshelf::updateFFmpegFlags() {
    ...
    ffmpegFlags = QString("tiltshelf=frequency=%1:width_type=%2:width=%3:gain=%4:poles=%5:mix=%6:normalize=%7:transform=%8:precision=%9")
...
QString FFTiltshelf::buildFFmpegFlags() const {
    return ffmpegFlags;
}

void FFTiltshelf::toJSON(QJsonObject& json) const {
    json["type"] = "ff-tiltshelf";
...
void FFTiltshelf::fromJSON(const QJsonObject& json) {
