...
FFBandpass::FFBandpass() {
    position = Position::MIDDLE;
    updateFFmpegFlags();
}

QWidget* FFBandpass::getParametersWidget() {
    ...
    // Constant Skirt Gain
    auto csgGroup = new QGroupBox("Skirt Gain");
    auto csgLayout = new QVBoxLayout(csgGroup);

    csgCheck = new QCheckBox("Constant skirt gain (peak gain = Q)");
    csgCheck->setChecked(m_csg);

    csgLayout->addWidget(csgCheck);
    filterLayout->addWidget(csgGroup);
    ...
    connect(csgCheck, &QCheckBox::toggled, [this](bool checked) {
        m_csg = checked;
        updateFFmpegFlags();
    });
...
void FFBandpass::updateFFmpegFlags() {
    ...
    ffmpegFlags = QString("bandpass=frequency=%1:width_type=%2:width=%3:csg=%4:mix=%5:normalize=%6:transform=%7:precision=%8")
        .arg(m_frequency, 0, 'f', 2)
        .arg(widthTypeName)
        .arg(m_width, 0, 'f', 4)
        .arg(m_csg ? "true" : "false")
        .arg(m_mix, 0, 'f', 2)
...
QString FFBandpass::buildFFmpegFlags() const {
    return ffmpegFlags;
}

void FFBandpass::toJSON(QJsonObject& json) const {
    json["type"] = "ff-bandpass";
