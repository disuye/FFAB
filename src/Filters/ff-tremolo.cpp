...
QWidget* FFTremolo::getParametersWidget() {
...
void FFTremolo::updateFFmpegFlags() {
    ffmpegFlags = QString("tremolo=f=%1:d=%2")
                      .arg(m_frequency, 0, 'f', 1)
                      .arg(m_depth, 0, 'f', 2);
}

QString FFTremolo::buildFFmpegFlags() const {
    return ffmpegFlags;
}

void FFTremolo::toJSON(QJsonObject& json) const {
    json["type"] = "ff-tremolo";
    json["frequency"] = m_frequency;
    json["depth"] = m_depth;
}

void FFTremolo::fromJSON(const QJsonObject& json) {
