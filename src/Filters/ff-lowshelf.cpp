...
FFLowshelf::FFLowshelf() {
    position = Position::MIDDLE;
    updateFFmpegFlags();
}

QWidget* FFLowshelf::getParametersWidget() {
    ...
    // ========== TAB 1: SHELF ==========
    auto shelfTab = new QWidget();
    auto shelfLayout = new QVBoxLayout(shelfTab);
    ...
    frequencySpinBox->setSingleStep(5.0);
    ...
    gainSpinBox->setMinimum(-24.0);
    gainSpinBox->setMaximum(24.0);
    ...
    widthTypeCombo->setCurrentIndex(m_widthType - 1);

    widthTypeLayout->addWidget(widthTypeCombo);
    shelfLayout->addWidget(widthTypeGroup);
    ...
    shelfLayout->addWidget(polesGroup);

    shelfLayout->addStretch();
    tabWidget->addTab(shelfTab, "Shelf");
    ...
    precisionCombo->setCurrentIndex(m_precision + 1);
    ...
    connect(frequencySlider, &QSlider::valueChanged, [this](int value) {
        m_frequency = sliderToFreq(value, 20.0, 500.0);
    ...
    connect(frequencySpinBox, QOverload<double>::of(&QDoubleSpinBox::valueChanged), [this](double value) {
        ...
        frequencySlider->setValue(freqToSlider(value, 20.0, 500.0));
    ...
    connect(gainSpinBox, QOverload<double>::of(&QDoubleSpinBox::valueChanged), [this](double value) {
        ...
        gainSlider->setValue(static_cast<int>(std::clamp(value * 100.0, -2400.0, 2400.0)));
    ...
    connect(widthSpinBox, QOverload<double>::of(&QDoubleSpinBox::valueChanged), [this](double value) {
        ...
        widthSlider->setValue(static_cast<int>(std::clamp(value * 100.0, 1.0, 500.0)));
...
void FFLowshelf::updateFFmpegFlags() {
    ...
    ffmpegFlags = QString("lowshelf=frequency=%1:width_type=%2:width=%3:gain=%4:poles=%5:mix=%6:normalize=%7:transform=%8:precision=%9")
...
QString FFLowshelf::buildFFmpegFlags() const {
    return ffmpegFlags;
}

void FFLowshelf::toJSON(QJsonObject& json) const {
    json["type"] = "ff-lowshelf";
...
void FFLowshelf::fromJSON(const QJsonObject& json) {
    m_frequency = json["frequency"].toDouble(100.0);
    m_widthType = json["width_type"].toInt(2);
    m_width = json["width"].toDouble(0.5);
...
void FFLowshelf::resetParametersWidget() {
