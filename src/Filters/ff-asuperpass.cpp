...
FFAsuperpass::FFAsuperpass() {
    position = Position::MIDDLE;
    updateFFmpegFlags();
}

QWidget* FFAsuperpass::getParametersWidget() {
    ...
    auto infoLabel = new QLabel(
        ...
        "Passes frequencies around the center, attenuates both low and high."
    );
    ...
    // Q Factor (0.01 - 100)
    auto qfactorGroup = new QGroupBox("Q Factor (Bandwidth)");
    ...
    auto qfactorHint = new QLabel("Higher Q = narrower bandwidth");
    ...
    auto orderHint = new QLabel("Higher = steeper rolloff at band edges");
...
void FFAsuperpass::updateFFmpegFlags() {
    ffmpegFlags = QString("asuperpass=centerf=%1:order=%2:qfactor=%3:level=%4")
                      .arg(m_centerf, 0, 'f', 1)
                      .arg(m_order)
                      .arg(m_qfactor, 0, 'f', 2)
                      .arg(m_level, 0, 'f', 2);
...
QString FFAsuperpass::buildFFmpegFlags() const {
    return ffmpegFlags;
}

void FFAsuperpass::toJSON(QJsonObject& json) const {
    json["type"] = "ff-asuperpass";
...
void FFAsuperpass::fromJSON(const QJsonObject& json) {
