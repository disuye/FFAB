...
FFAfreqshift::FFAfreqshift() {
    position = Position::MIDDLE;
    updateFFmpegFlags();
}

QWidget* FFAfreqshift::getParametersWidget() {
    ...
    connect(shiftSlider, &QSlider::valueChanged, [this](int value) {
        m_shift = static_cast<double>(value);
    ...
    connect(shiftSpinBox, QOverload<double>::of(&QDoubleSpinBox::valueChanged), [this](double value) {
        ...
        // Clamp slider to practical range
        int sliderValue = static_cast<int>(value);
        if (sliderValue < -10000) sliderValue = -10000;
        if (sliderValue > 10000) sliderValue = 10000;
        
        shiftSlider->blockSignals(true);
        shiftSlider->setValue(sliderValue);
...
void FFAfreqshift::updateFFmpegFlags() {
    ffmpegFlags = QString("afreqshift=shift=%1:level=%2:order=%3")
                      .arg(m_shift, 0, 'f', 1)
                      .arg(m_level, 0, 'f', 2)
                      .arg(m_order);
}

QString FFAfreqshift::buildFFmpegFlags() const {
    return ffmpegFlags;
}

void FFAfreqshift::toJSON(QJsonObject& json) const {
    json["type"] = "ff-afreqshift";
    json["shift"] = m_shift;
    json["level"] = m_level;
    json["order"] = m_order;
}

void FFAfreqshift::fromJSON(const QJsonObject& json) {
...
void FFAfreqshift::resetParametersWidget() {
