...
/**
 * FFBandreject - Wraps FFmpeg's 'bandreject' audio filter
 * Apply a two-pole Butterworth band-reject (notch) filter
 * FFmpeg filter: bandreject=frequency=<f>:width_type=<t>:width=<w>:mix=<m>:...
 */
class FFBandreject : public BaseFilter {
    Q_OBJECT
public:
    FFBandreject();
    ~FFBandreject() override = default;

    QString displayName() const override { return "Band Reject (Notch)"; }
    QString filterType() const override { return "ff-bandreject"; }
