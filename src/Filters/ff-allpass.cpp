...
QWidget* FFAllpass::getParametersWidget() {
    ...
    orderCombo->addItem("1st Order (90° phase shift)", 1);
    orderCombo->addItem("2nd Order (180° phase shift)", 2);
    orderCombo->setCurrentIndex(m_order - 1);

    orderLayout->addWidget(orderCombo);
    filterLayout->addWidget(orderGroup);
    ...
    connect(orderCombo, QOverload<int>::of(&QComboBox::currentIndexChanged), [this](int index) {
        m_order = orderCombo->itemData(index).toInt();
        updateFFmpegFlags();
    });
...
void FFAllpass::updateFFmpegFlags() {
    ...
    ffmpegFlags = QString("allpass=frequency=%1:width_type=%2:width=%3:order=%4:mix=%5:normalize=%6:transform=%7:precision=%8")
...
QString FFAllpass::buildFFmpegFlags() const {
    return ffmpegFlags;
}

void FFAllpass::toJSON(QJsonObject& json) const {
    json["type"] = "ff-allpass";
