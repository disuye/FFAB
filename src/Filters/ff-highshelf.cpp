...
FFHighshelf::FFHighshelf() {
    position = Position::MIDDLE;
    updateFFmpegFlags();
}

QWidget* FFHighshelf::getParametersWidget() {
    ...
    frequencySpinBox->setSingleStep(100.0);
    ...
    auto frequencyMinLabel = new QLabel("1k");
    ...
    frequencySlider->setValue(freqToSlider(m_frequency, 1000.0, 20000.0));
    ...
    connect(frequencySlider, &QSlider::valueChanged, [this](int value) {
        m_frequency = sliderToFreq(value, 1000.0, 20000.0);
    ...
    connect(frequencySpinBox, QOverload<double>::of(&QDoubleSpinBox::valueChanged), [this](double value) {
        ...
        frequencySlider->setValue(freqToSlider(value, 1000.0, 20000.0));
...
void FFHighshelf::updateFFmpegFlags() {
    ...
    ffmpegFlags = QString("highshelf=frequency=%1:width_type=%2:width=%3:gain=%4:poles=%5:mix=%6:normalize=%7:transform=%8:precision=%9")
...
QString FFHighshelf::buildFFmpegFlags() const {
    return ffmpegFlags;
}

void FFHighshelf::toJSON(QJsonObject& json) const {
    json["type"] = "ff-highshelf";
...
void FFHighshelf::fromJSON(const QJsonObject& json) {
