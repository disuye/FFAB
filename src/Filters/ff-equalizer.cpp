...
QWidget* FFEqualizer::getParametersWidget() {
    ...
    auto frequencyGroup = new QGroupBox("Center Frequency (Hz)");
    ...
    eqLayout->addWidget(frequencyGroup);
    ...
    eqLayout->addWidget(widthTypeGroup);

    // Width / Q
    auto widthGroup = new QGroupBox("Width / Q");
...
void FFEqualizer::updateFFmpegFlags() {
    ...
    ffmpegFlags = QString("equalizer=frequency=%1:width_type=%2:width=%3:gain=%4:mix=%5:normalize=%6:transform=%7:precision=%8")
...
QString FFEqualizer::buildFFmpegFlags() const {
    return ffmpegFlags;
}

void FFEqualizer::toJSON(QJsonObject& json) const {
    json["type"] = "ff-equalizer";
...
void FFEqualizer::fromJSON(const QJsonObject& json) {
    m_frequency = json["frequency"].toDouble(1000.0);
